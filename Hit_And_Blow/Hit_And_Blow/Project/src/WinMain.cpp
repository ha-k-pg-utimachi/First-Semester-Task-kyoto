...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    for (int i = 0; i < 4; i++)// 配列 num を for文 で初期化
    {
        num[i] = i;
    }
    // ゲームのメインループ
...
// --- 入力状況をチェックして、適切な処理を行う
        ...
        if (gameClear == false)
        {
            ...
            else if (IsPushKey(MY_INPUT_RIGHT))
            {
                ...
                if (cursor > DIGITS - 1)// 増やした結果、(DIGITS - 1) より大きくなった場合は、(DIGITS - 1)にする
                {
                    cursor = DIGITS - 1;
                }
...
void CreateTargetNumber()
{
    srand((unsigned int)time(NULL));// 乱数をtime()関数で初期化
    ...
    *numBlow = 0;
    ...
    for (int i = 0; i < DIGITS; i++)
    {
        for (int j = 0; j < DIGITS; j++)
        {
            ...
            if (target[i] == num[j])
            {
                if (i == j)// 当てるべき数が num に入っていることが分かったので、Hit か Blow かを調べる
                {
                    *numHit += 1;    // 配列の同じ位置にある場合は numHit のアドレスの中身を 1 増やす
                }
                else
                {
                    numBlow += 1;
